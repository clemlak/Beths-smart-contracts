...
contract('Beths', (accounts) => {
  describe('Testing a disputed bet', () => {
    ...
    it('Should solve the dispute', () => beths.solveDispute(0, Status.Won, {
      from: accounts[3],
    }));
    ...
    it('Should check the balance of account 1', () => testToken.balanceOf(accounts[1])
      .then((balance) => {
        assert.equal(balance.toString(), Utils.toWei('96'), 'Account 1 balance is wrong');
      }));
    ...
    it('Should check the balance of account 3', () => testToken.balanceOf(accounts[3])
      .then((balance) => {
        assert.equal(balance.toString(), Utils.toWei('2'), 'Account 3 balance is wrong');
      }));
