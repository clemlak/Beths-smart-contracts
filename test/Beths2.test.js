...
contract('Beths', (accounts) => {
  describe('Testing a lost bet', () => {
    ...
    it('Should check the balance of account 1', () => testToken.balanceOf(accounts[1])
      .then((balance) => {
        assert.equal(balance.toString(), Utils.toWei('0'), 'Account 1 balance is wrong');
      }));

    it('Should check the balance of account 2', () => testToken.balanceOf(accounts[2])
      .then((balance) => {
        assert.equal(balance.toString(), Utils.toWei('98'), 'Account 2 balance is wrong');
      }));
